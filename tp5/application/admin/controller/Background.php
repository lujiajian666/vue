...
class Background extends Base
{
    ...
    public function addEmployee(){
        ...
        $arr = array(
            ...
            "username" => $this->pinyin($post["name"]).substr(time(), 5)
        );
    ...
    public function getEmployee(){
        ...
        $count = Db::name($this->tableEmployee)->alias("a")
             ...
             ->where("a.department_id=$post[department]")
             ->count();  
    ...
    public function selectJobTitleByDepartment(){
        $post=$this->request->post();
        $data = Db::name($this->tableJobTitle)->where(["department_id"=>$post["departmentid"]])
        ->order("department_id asc")->select();
        return json($data);
    }
    ...
    public function resetPass(){
        $this->authorityVerify();
        $post=$this->request->post();
        $db=Db::name($this->tableEmployee);
        $res=$db->where(["id"=>$post["id"]])->update(["password"=>"admin"]);
        if($res!==false){
            return json(["status"=>1]);
        }else{
            return json(["status"=>0,"txt"=>$db->getLastSql()]);
        }
    }
    public function changePass(){
        ...
        $db=Db::name($this->tableEmployee);
        $res=$db->where(["username"=>$_SESSION["username"]])->update(["password"=>$password]);
        if($res!==false){
            return json(["status"=>1]);
        }else{
            return json(["status"=>0,"txt"=>$db->getLastSql()]);
